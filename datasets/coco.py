...
class COCODataSets(Dataset):
    ...
    def set_transform(self):
        ...
        basic_transform = Compose(
            transforms=[
                color_gitter,
                RandCrop(min_thresh=0.6, max_thresh=1.0).reset(p=0.1),
                RandScaleMinMax(min_threshes=self.min_threshes, max_thresh=self.max_thresh, padding=True),
        ...
        std_transform = RandScaleMinMax(min_threshes=self.min_threshes[-1:], max_thresh=self.max_thresh)
    ...
    @staticmethod
    def collect_fn_seg(batch: List[BoxSegInfo]):
        batch_img = list()
        batch_labels = list()
        batch_mask = list()
        batch_length = list()
        for item in batch:
            ...
            assert len(item.labels) == len(item.mask)
            batch_length.append(len(item.labels))
        batch_img = torch.from_numpy(np.stack(batch_img)).permute(0, 3, 1, 2).float()
        batch_labels = torch.from_numpy(np.concatenate(batch_labels)).long()
        batch_mask = torch.from_numpy(np.concatenate(batch_mask)).float()
        return batch_img, batch_labels, batch_mask, batch_length
...
if __name__ == '__main__':
    ...
    loader = DataLoader(dataset=coco_data_sets, batch_size=2, shuffle=True, collate_fn=coco_data_sets.collect_fn_seg)
