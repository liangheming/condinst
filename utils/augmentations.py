...
class RandScaleMinMax(BasicTransform):
    def __init__(self, min_threshes, max_thresh=1024, padding=False, **kwargs):
        kwargs['p'] = 1.0
    ...
    def aug(self, box_seg_info: BoxSegInfo) -> BoxSegInfo:
        ...
        h, w = img.shape[:2]
        if self.padding:
            img = cv.copyMakeBorder(img, 0, self.max_thresh - h, 0, self.max_thresh - w,
                                    cv.BORDER_CONSTANT, value=box_seg_info.padding_val)
        ...
        if box_seg_info.mask is not None and len(box_seg_info.mask):
            ...
            if self.padding:
                mask = cv.copyMakeBorder(mask, 0, self.max_thresh - h, 0, self.max_thresh - w,
...
class Mosaic(BasicTransform):
    ...
    def aug(self, box_info: BoxSegInfo) -> BoxSegInfo:
        ...
        if len(box_info4):
            ...
            label_4 = list()
            mask_4 = list()
            for item in box_info4:
                box_4.append(item.boxes)
                if item.labels is not None:
                    label_4.append(item.labels)
                mask_4.append(item.mask)
            box_4 = np.concatenate(box_4, axis=0)
        ...
        # tic3 = time.time()
        ret = self.affine(box_info)
        # tic4 = time.time()
        # print("all ", tic3 - tic, " four ", tic1 - tic, " cat ", tic2 - tic1, " filter ", tic3 - tic2, ' affine ',
        #       tic4 - tic3)
        return ret
